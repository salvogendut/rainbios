...
"""Run the BBC BASIC cassette LOAD/RUN integration probe against the internal
embedded payload under RainBIOS in 1983."""
...
def validate_state(text: str) -> dict[str, str]:
    ...
    try:
        ...
        raise ValueError("1983 emitted an invalid embedded tape state") from error
    if pc != 0x4400:
        raise ValueError(
...
def main() -> int:
    ...
    parser.add_argument("--exit-after", type=int, default=3000)
    ...
    try:
        ...
        print(f"error: invalid 1983 embedded tape state: {error}", file=sys.stderr)
        return 1
    print(
        "validated embedded payload cassette LOAD/RUN in 1983: "
