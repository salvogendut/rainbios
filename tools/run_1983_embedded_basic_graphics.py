...
"""Run the BBC BASIC graphics program against the internal embedded payload
under RainBIOS in 1983."""
...
def validate_state(text: str) -> dict[str, str]:
    ...
    try:
        ...
        raise ValueError("1983 emitted an invalid embedded graphics state") from error
    ...
    expected = {"slot": "FC", "vdp_r0": "02", "vdp_r1": "E0"}
...
def main() -> int:
    ...
    try:
        ...
        print(f"error: invalid 1983 embedded graphics state: {error}", file=sys.stderr)
        return 1
    print(
        "validated 1983 embedded payload graphics state: "
