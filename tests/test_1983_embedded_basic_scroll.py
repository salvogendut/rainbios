...
from tools.run_1983_embedded_basic_scroll import parse_marker, validate_state
...
class EmbeddedBasicScrollTests(unittest.TestCase):
    ...
    def test_slot_must_be_the_payload_ram(self) -> None:
        bad = GOOD.replace("slot=FC", "slot=F4")
        with self.assertRaises(ValueError):
            validate_state(bad)
