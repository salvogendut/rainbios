...
class VdpStateProbeTests(unittest.TestCase):
        ...
        with self.assertRaisesRegex(ValueError, "BOOT"):
            validate_report(
                self.make_report().replace(
                    "BOOT=02,E0,06,FF,03,36,07,01,02,E0,06,FF,03,36,07,B4,1800,0000,3800,1B00,02,20",
                    "BOOT=02,E0,06,FF,03,36,07,01,02,A0,06,FF,03,36,07,B4,1800,0000,3800,1B00,02,20",
                )
            )
    ...
    def test_initxt_state(self) -> None:
        with self.assertRaisesRegex(ValueError, "INITXT"):
            validate_report(
                self.make_report().replace(
                    "INITXT=00,F0,00,00,01,36,07,B4,00,F0,00,00,01,36,07,B4,0000,0800,0000,0000,00,28",
                    "INITXT=00,F0,00,00,01,36,07,B4,00,F0,00,00,01,36,07,B4,0000,0800,0000,0000,02,28",
                )
            )
