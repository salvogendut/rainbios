...
from tools.run_1983_embedded_basic_edit import parse_markers, validate_state


GOOD = (
    "state frame=6001 pc=5432 sp=F300 slot=FC subslot=00 "
...
class EmbeddedBasicEditTests(unittest.TestCase):
    def test_edit_state_is_accepted(self) -> None:
        self.assertEqual(validate_state(GOOD)["slot"], "FC")
    ...
    def test_slot_must_be_the_payload_ram(self) -> None:
        with self.assertRaises(ValueError):
            validate_state(GOOD.replace("slot=FC", "slot=F4"))
