...
TEST_STATE      equ #f398
                ...
                db #41,#42
                dw menu_disk2_input_init
                dw 0,0,0
                defs 6,0

menu_disk2_input_init:
                xor a
                ld (TEST_STATE),a
                ld hl,menu_disk2_input_hook_template
                ld de,HOOK_TIMI
                ld bc,5
                ldir
                ret

menu_disk2_input_hook_template:
                db #f7                         ; RST 30h / CALLF
                db 2                           ; primary slot 2
                dw menu_disk2_input_tick
                db #c9                         ; RET after inline operands

menu_disk2_input_tick:
                ld a,(TEST_STATE)
                cp #ff
                ret z
                inc a
                ld (TEST_STATE),a
                cp 2
                jr z,menu_disk2_input_space
                cp 20
                ret nz
                ld a,#ff
                ld (TEST_STATE),a
                ld a,'2'
                jr menu_disk2_input_put
menu_disk2_input_space:
                ld a,#20
menu_disk2_input_put:
                ld hl,(PUTPNT)
                ld (hl),a
                inc hl
                ld (PUTPNT),hl
