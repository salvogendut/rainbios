...
class ScreenModesProbeTests(unittest.TestCase):
    ...
    def test_settxt_must_switch_to_screen_zero(self) -> None:
        with self.assertRaisesRegex(ValueError, "SETTXT"):
            validate_report(
                self.make_report().replace("SETTXT=00,F0,00,00,01,36,07,BC,00,28,55,01,01",
    ...
    def test_settxt_must_not_clear_the_tables(self) -> None:
        with self.assertRaisesRegex(ValueError, "SETTXT"):
            validate_report(
                self.make_report().replace("SETTXT=00,F0,00,00,01,36,07,BC,00,28,55,01,01",
                                           "SETTXT=00,F0,00,00,01,36,07,BC,00,28,20,01,01")
            )
    ...
    def test_settxt_must_home_the_cursor(self) -> None:
        with self.assertRaisesRegex(ValueError, "SETTXT"):
            validate_report(
                self.make_report().replace("SETTXT=00,F0,00,00,01,36,07,BC,00,28,55,01,01",
                                           "SETTXT=00,F0,00,00,01,36,07,BC,00,28,55,07,09")
            )
        ...
        with self.assertRaisesRegex(ValueError, "SETT32"):
            validate_report(
                self.make_report().replace("SETT32=00,E0,06,80,00,36,07,F5,01,20",
                                           "SETT32=00,F0,00,00,01,36,07,BC,00,28")
            )
