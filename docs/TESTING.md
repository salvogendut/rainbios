...
| `test-openmsx-vram` | VRAM transfer calls with hardening: WRTVRM/RDVRM round trips, the 14-bit address wrap at `4000h` (WRAPTOP/BASE/ZERO), `FILVRM`/`LDIRVM`/`LDIRMV` crossing the boundary, and port-ordering — an H.TIMI hook writes R0 every VBlank while 4000 `WRTVDP` calls to R7 complete, so R7 must stay clean (`ORDER`), the hook must have fired (`HOOKFIRE`), and the registers must remain readable (`VDPREG`) |
