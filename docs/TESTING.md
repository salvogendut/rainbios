...
| `test-openmsx-mapper` | Boot-time memory-mapper sizing for a complete 4 MiB mapper and an Omega-style 512 KiB installation with decoded but physically absent upper SRAM banks; also checks access to a segment beyond the fixed 64 KiB baseline |
...
| `test-1983-embedded-basic-floppy` | Embedded BASIC saves `TEST.BBC` to a writable FAT12 disk, lists it and the FAT-derived free space with `*CAT` and `*DIR` (including an odd entry whose allocation value uses the high nibble), restarts, CHAINs the persisted program, proves LOAD/catalogue are read-only, and checks explicit write-protect/no-media errors |
