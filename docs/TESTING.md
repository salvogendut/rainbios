...
| `test-openmsx-keyboard` | Physical matrix input, translation, CAPS, buffering, blocking `CHGET`, Ctrl-STOP break, function-key display flags, text-mode forcing, auto-repeat, and `INLIN`/`PINLIN`/`QINLIN`/`BEEP` |
