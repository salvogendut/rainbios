...
On 2026-09-10, a black pixel-art representation of the rain kanji `雨` was
added above the frog from a reference image supplied by the repository owner.
The reference silhouette was thresholded, nearest-neighbour resampled to a
24×23 hard-edged mask, and composited with ImageMagick at `(48,32)`. The
original dimensions and background colour were preserved, and pixels outside
that 24×23 rectangle are unchanged. The reference image is not redistributed.
