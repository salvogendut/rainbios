...
EMULATOR_1983_EMBEDDED_BASIC_SCREEN := \
 ...
 EMULATOR_1983_BBC_SCROLL_SCREEN := \
...
$(DISK_DSKFMT_IMAGE): tools/make_fat12_disk.py $(DISK_DSKFMT_SECTOR_BIN)
	$(PYTHON) $< --boot-sector $(DISK_DSKFMT_SECTOR_BIN) $@
...
test-openmsx-bbcbasic-quote: $(OPENMSX_MACHINE)
	mkdir -p $(OPENMSX_HOME) $(OPENMSX_M1_REPORT_DIR)
	OPENMSX_HOME=$(abspath $(OPENMSX_HOME)) \
	OPENMSX_USER_DATA=$(abspath $(OPENMSX_SHARE)) \
	$(OPENMSX) -machine RainBIOS_MSX1 \
		-command "set bbcbasic_quote_output {$(abspath $(OPENMSX_BBC_QUOTE_REPORT))}" \
...
test-1983-disk-abi: $(MSX1_ROM) $(DISK_ABI_PROBE_CART)
	mkdir -p $(EMULATOR_1983_DIR)
...
test-1983-inifnk: $(MSX1_ROM) $(INIFNK_PROBE_CART)
	mkdir -p $(EMULATOR_1983_DIR)
	$(PYTHON) tools/run_1983_inifnk_probe.py \
		--emulator "$(EMULATOR_1983)" --models "$(MODELS_1983)" \
		--bios "$(MSX1_ROM)" --cartridge "$(INIFNK_PROBE_CART)"

test-1983-iscntc: $(MSX1_ROM) $(ISCNTC_PROBE_CART)
	mkdir -p $(EMULATOR_1983_DIR)
	$(PYTHON) tools/run_1983_iscntc_probe.py \
		--emulator "$(EMULATOR_1983)" --models "$(MODELS_1983)" \
		--bios "$(MSX1_ROM)" --cartridge "$(ISCNTC_PROBE_CART)"

test-1983-chgmod: $(MSX1_ROM) $(CHGMOD_PROBE_CART)
	mkdir -p $(EMULATOR_1983_DIR)
...
test-1983-embedded-basic-graphics: \
	...
	mkdir -p $(EMULATOR_1983_DIR)
	$(PYTHON) tools/run_1983_embedded_basic_graphics.py \
		--emulator "$(EMULATOR_1983)" --models "$(MODELS_1983)" \
		--bios "$(MSX1_ROM)" \
...
test-1983-bbcbasic-scroll: \
		$(MSX1_ROM) $(BBC_BASIC_ROM) $(SCROLL_INPUT_CART)
	mkdir -p $(EMULATOR_1983_DIR)
	$(PYTHON) tools/run_1983_bbcbasic_scroll.py \
		--emulator "$(EMULATOR_1983)" --models "$(MODELS_1983)" \
		--bios "$(MSX1_ROM)" --cartridge "$(BBC_BASIC_ROM)" \
		--input-cartridge "$(SCROLL_INPUT_CART)" \
		--screenshot "$(EMULATOR_1983_BBC_SCROLL_SCREEN)"

test-1983-embedded-basic-scroll: \
		$(MSX1_ROM) $(SCROLL_INPUT_CART)
	mkdir -p $(EMULATOR_1983_DIR)
	$(PYTHON) tools/run_1983_embedded_basic_scroll.py \
		--emulator "$(EMULATOR_1983)" --models "$(MODELS_1983)" \
		--bios "$(MSX1_ROM)" \
		--input-cartridge "$(SCROLL_INPUT_CART)" \
...
test-1983-disk-dskfmt: \
		$(MSX1_ROM) $(NMS8250_DISK_ROM) $(DISK_DSKFMT_IMAGE)
	mkdir -p $(EMULATOR_1983_DIR)
	$(PYTHON) tools/run_1983_disk_dskfmt.py \
		--emulator "$(EMULATOR_1983)" --models "$(MODELS_1983)" \
		--bios "$(MSX1_ROM)" --disk-rom "$(NMS8250_DISK_ROM)" \
		--disk-a "$(DISK_DSKFMT_IMAGE)" \
		--screenshot "$(EMULATOR_1983_DISK_DSKFMT_SCREEN)"
